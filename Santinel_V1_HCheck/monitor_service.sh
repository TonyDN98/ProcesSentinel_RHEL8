...
check_dependencies() {
    # Check for bc (used for backoff calculations)
    if ! command -v bc &> /dev/null; then
        log "WARNING" "bc command not found. Exponential backoff will use simple multiplication."
        # Define a fallback function for multiplication
        bc_multiply() {
            echo $(($1 * $2))
        }
    else
        # Use bc for more precise calculations
        bc_multiply() {
            echo "scale=0; $1 * $2" | bc
        }
    fi

    # Export the function for use in the script
    export -f bc_multiply
}
...
# Execute health check after restart
execute_post_health_check() {
    local process_name="$1"
    local health_check=$(get_process_config "$process_name" "post_health_check" "")

    if [ -n "$health_check" ]; then
        log "INFO" "Executing post-restart health check for $process_name: $health_check"
        if eval "$health_check"; then
            log "INFO" "Post-restart health check passed for $process_name"
            return 0
        else
            log "ERROR" "Post-restart health check failed for $process_name"
            return 1
        fi
    fi
    # If no health check is configured, assume restart was successful
    return 0
}
...
restart_process() {
    ...
    local use_backoff=$(get_process_config "$process_name" "use_backoff" "false")
    local backoff_multiplier=$(get_process_config "$process_name" "backoff_multiplier" "2")
    ...
    while [ $attempt -le "$max_attempts" ]; do
        ...
        case "$strategy" in
            "service")
                ...
                if systemctl restart "$process_name" 2>/dev/null; then
                    log "INFO" "RESTART LOG: Service restart command succeeded for: $process_name"
            ...
            "process")
                ...
                if pkill "$process_name"; then
                    ...
                    if "$process_name" >/dev/null 2>&1 & then
                        log "INFO" "RESTART LOG: Process start command succeeded for: $process_name"
                        restart_success=true
                    fi
                fi
                ;;
        ...
        # Verify restart success with post-health check
        if [ "$restart_success" = true ]; then
            # Give the service a moment to fully initialize
            sleep 2

            if execute_post_health_check "$process_name"; then
                log "INFO" "RESTART LOG: Post-health check passed for $process_name"
                return 0
            else
                log "ERROR" "RESTART LOG: Post-health check failed for $process_name"
        ...
        # Calculate next delay with backoff if enabled
        if [ "$use_backoff" = "true" ]; then
            current_delay=$(bc_multiply $current_delay $backoff_multiplier)
            log "INFO" "RESTART LOG: Using backoff strategy, next delay: $current_delay seconds"
        fi
